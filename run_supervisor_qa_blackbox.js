...
const SCREENSHOTS_DIR = path.join(__dirname, 'screenshots_qa_supervisor');
...
async function runSupervisorQABlackbox() {
    console.log("Starting Ultimate Full E2E Supervisor QA Blackbox Tests...");
    ...
    try {
        ...
        await page.type('input[name="email"]', 'ranggamardika941@gmail.com');
        ...
        results.push(makeResult('TC1.2', 'Authentication', 'Login', 'Negatif', 'Form Login', 'Isi email valid, password salah', 'ranggamardika941@gmail.com / salahpassword', 'Gagal login, muncul alert', 'Gagal dan alert muncul', 'Pass', 'TC1.2_Neg.png'));
        ...
        results.push(makeResult('TC1.3', 'Authentication', 'Login', 'Positif', 'Form Login', 'Isi kredensial benar', 'ranggamardika941@gmail.com', 'Login sukses, dialihkan ke Dashboard', 'Sukses dialihkan', 'Pass', 'TC1.3_Pos.png'));
        ...
        await page.goto(`${BASE_URL}/supervisor-dashboard`, { waitUntil: 'networkidle0' });
        await page.screenshot({ path: path.join(SCREENSHOTS_DIR, 'TC2.1_Pos.png') });
        results.push(makeResult('TC2.1', 'Navigation', 'Dashboard', 'Positif', 'Login sbg Supervisor', 'Buka Dashboard', '-', 'Halaman Dashboard tampil', 'Tampil sukses', 'Pass', 'TC2.1_Pos.png'));
        ...
        // Create (Positive)
        const t=await page.$('input[name="title"]'); if(!t) { console.log(await page.url()); console.log(await page.content()); } await page.type('input[name="title"]', 'QA Brief Testing Campaign');
        await page.evaluate(() => {
...
async function generateExcel(data) {
    ...
    const worksheet = workbook.addWorksheet('Supervisor QA Matrix');
    ...
    const reportPath = path.join(__dirname, 'Supervisor_QA_Report.xlsx');
    await workbook.xlsx.writeFile(reportPath);
}

runSupervisorQABlackbox();
