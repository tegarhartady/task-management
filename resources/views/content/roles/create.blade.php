      ...
      <div class="card-body">
        <form action="{{ route('roles.store') }}" method="POST">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="E.g. Marketing Manager" value="{{ old('name') }}" required>
            @error('name')
          ...
          <div class="mb-4">
            ...
            <textarea id="description" name="description" class="form-control @error('description') is-invalid @enderror" placeholder="Brief explanation about what this role does">{{ old('description') }}</textarea>
            @error('description')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="row g-3">
            @foreach($menus as $menu)
              ...
              <div class="col-md-6">
                <div class="form-check form-check-primary mt-3">
                  <input class="form-check-input" type="checkbox" name="menus[]" value="{{ $menu['slug'] }}" id="menu_{{ $menu['slug'] }}" 
                    {{ is_array(old('menus')) && in_array($menu['slug'], old('menus')) ? 'checked' : '' }}>
              ...
              @endif
            @endforeach
            <!-- Include default dashboard implicitly based on role or explicit selection? 
                 Usually pages-home is accessible by default. -->
            <div class="col-md-6">
                <div class="form-check form-check-primary mt-3">
                  <input class="form-check-input" type="checkbox" name="menus[]" value="pages-home" id="menu_pages-home" 
                    {{ is_array(old('menus')) && in_array('pages-home', old('menus')) ? 'checked' : 'checked' }}>
            ...
            <div class="col-md-6">
                <div class="form-check form-check-primary mt-3">
                  <input class="form-check-input" type="checkbox" name="menus[]" value="admin.dashboard" id="menu_admin-dashboard" 
                    {{ is_array(old('menus')) && in_array('admin.dashboard', old('menus')) ? 'checked' : '' }}>
